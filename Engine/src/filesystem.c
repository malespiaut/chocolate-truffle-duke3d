...
int32_t
initgroupfile(const char* filename)
{
  uint8_t buf[16];
  int32_t i, j, k;
...
void
crc32_table_gen(unsigned int* crc32_table) /* build CRC32 table */
{
...
unsigned int
crc32(uint8_t* buf, unsigned int length)
{
...
unsigned int
crc32_update(uint8_t* buf, unsigned int length, unsigned int crc_to_update)
{
...
int32_t
kopen4load(const char* filename, int openOnlyFromGRP)
{
...
int32_t
kread(int32_t handle, void* buffer, int32_t leng)
{
...
int
kread16(int32_t handle, short* buffer)
{
...
int
kread32(int32_t handle, int32_t* buffer)
{
...
int
kread8(int32_t handle, uint8_t* buffer)
{
...
int32_t
klseek(int32_t handle, int32_t offset, int whence)
{
...
int32_t
compress(uint8_t* lzwinbuf, int32_t uncompleng, uint8_t* lzwoutbuf)
{
...
int32_t
uncompress(uint8_t* lzwinbuf, int32_t compleng, uint8_t* lzwoutbuf)
{
...
void
kdfread(void* buffer, size_t dasizeof, size_t count, int32_t fil)
{
  size_t i, j;
  int32_t k, kgoal;
  short leng;
  ...
  if (lzwbuf1 == NULL)
    allocache(&lzwbuf1, LZWSIZE + (LZWSIZE >> 4), &lzwbuflock[0]);
  if (lzwbuf2 == NULL)
    allocache((uint8_t**)&lzwbuf2, (LZWSIZE + (LZWSIZE >> 4)) * 2, &lzwbuflock[1]);
  if (lzwbuf3 == NULL)
    allocache((uint8_t**)&lzwbuf3, (LZWSIZE + (LZWSIZE >> 4)) * 2, &lzwbuflock[2]);
  if (lzwbuf4 == NULL)
    allocache(&lzwbuf4, LZWSIZE, &lzwbuflock[3]);
  if (lzwbuf5 == NULL)
    allocache(&lzwbuf5, LZWSIZE + (LZWSIZE >> 4), &lzwbuflock[4]);
  ...
  kread(fil, &leng, 2);
  kread(fil, lzwbuf5, (int32_t)leng);
  k = 0;
  ...
  for (i = 1; i < count; i++)
  {
    ...
    for (j = 0; j < dasizeof; j++)
      ptr[j + dasizeof] = (uint8_t)((ptr[j] + lzwbuf4[j + k]) & 255);
    k += dasizeof;
    ptr += dasizeof;
  }
...
dfwrite(void* buffer, size_t dasizeof, size_t count, FILE* fil)
{
  ...
  if (k > LZWSIZE - dasizeof)
  {
    ...
    k = 0;
    fwrite(&leng, 2, 1, fil);
    fwrite(lzwbuf5, (int32_t)leng, 1, fil);
  }

  for (i = 1; i < count; i++)
  {
    for (j = 0; j < dasizeof; j++)
      lzwbuf4[j + k] = (uint8_t)((ptr[j + dasizeof] - ptr[j]) & 255);
    k += dasizeof;
    if (k > LZWSIZE - dasizeof)
    {
      leng = (short)compress(lzwbuf4, k, lzwbuf5);
...
int32_t
TCkopen4load(const char* filename, int readfromGRP)
{
