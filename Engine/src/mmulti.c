...
static int32_t
getcrc(uint8_t* buffer, short bufleng)
{
  int32_t i, j;
...
dosendpackets(long other)
{
  ...
  if (outcntplc[other] == outcntend[other])
    return;
  ...
  for (i = 0; i < gcom->numbytes; i++)
    printf("%2x ", gcom->buffer[i]);
  printf("\n");
#endif

#if (SIMULATEERRORS != 0)
  if (!(rand() & SIMULATEERRORS))
    gcom->buffer[rand() % gcom->numbytes] = (rand() & 255);
  if (rand() & SIMULATEERRORS)
#endif
  {
...
void
sendpacket(int32_t other, const uint8_t* bufptr, int32_t messleng)
{
  long i = 0;
  long j = 0;
  ...
  if (bakpacketlen[other][(outcntend[other] - 1) & 255] == messleng)
  {
    ...
    for (i = messleng - 1; i >= 0; i--)
      if (bakpacketbuf[(i + j) & (BAKSIZ - 1)] != bufptr[i])
        break;
  }
...
int32_t
getoutputcirclesize(void)
{
  return (0);
}

short
getpacket(short* other, uint8_t* bufptr)
{
  long i, messleng;
  unsigned short dacrc;
...
void
genericmultifunction(int32_t other, const uint8_t* bufptr, int32_t messleng, int32_t command)
{
  if (numplayers < 2)
    return;
...
static int
send_udp_packet(int ip, short port, void* pkt, size_t pktsize)
{
  ...
  rc = sendto(udpsocket, pkt, pktsize, 0, (struct sockaddr*)&addr, sizeof(addr));
...
static void
process_udp_send_queue(void)
{
  /* Fill this in if the non-blocking socket becomes an issue. */
}

static int
get_udp_packet(int* ip, short* _port, void* pkt, size_t pktsize)
{
  int err = 0;
  struct sockaddr_in addr;
  short port;
  ...
  int rc = recvfrom(udpsocket, pkt, pktsize, 0, (struct sockaddr*)&addr, (int*)&fromlen);
  ...
  if (rc == -1) /* fill in the addr structure on error... */
  {
    ...
    recvfrom(udpsocket, NULL, 0, MSG_ERRQUEUE, (struct sockaddr*)&addr, &l);
  }
#endif
  ...
  *ip = ntohl(addr.sin_addr.s_addr);
  ...
  if (gcom == NULL)
  {
    ...
    {
      if ((allowed_addresses[itmp].host == *ip) && (allowed_addresses[itmp].port == port))
      {
  ...
  else
  {
    ...
    {
      if ((allowed_addresses[i].host == *ip) && (allowed_addresses[i].port == port))
...
static char*
read_whole_file(const char* cfgfile)
{
  char* buf;
  long len, rc;
  long handle;
...
static int
set_socket_blockmode(int onOrOff)
{
  unsigned long flags;
  int rc = 0;
...
static int
set_socket_broadcast(int onOrOff)
{
  int f = (onOrOff) ? 1 : 0;
  int rc;
...
static int
open_udp_socket(int ip, int port)
{
  struct sockaddr_in addr;
...
/* server init. */
static int
wait_for_other_players(gcomtype* gcom, int myip)
{
...
/* client init. */
static int
connect_to_server(gcomtype* gcom, int myip)
{
...
static void
send_peer_greeting(int ip, short port, short myid)
{
...
/* peer to peer init. */
static int
connect_to_everyone(gcomtype* gcom, int myip, int bcast)
{
  PacketPeerGreeting packet;
  ...
  memmove(&allowed_addresses[1], &allowed_addresses[0], sizeof(allowed_addresses) - sizeof(allowed_addresses[0]));
...
static int
parse_ip(const char* str, int* ip)
{
  int ip1, ip2, ip3, ip4;
...
static int
parse_interface(char* str, int* ip, short* udpport)
{
  char* ptr = strchr(str, ':');
...
static int
parse_udp_config(const char* cfgfile, gcomtype* gcom)
{
  char* buf;
  char* tok;
  char* ptr;
...
static gcomtype*
init_network_transport(char** ARGV, int argpos)
{
  gcomtype* retval;
...
static void
deinit_network_transport(gcomtype* gcom)
{
