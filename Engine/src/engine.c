...
static inline int32_t
getclipmask(int32_t a, int32_t b, int32_t c, int32_t d)
{ // Ken did this
  ...
  do
  {
    ...
    for (z = numscansbefore; z < numscans; z++)
    {
      if ((wall[pvWalls[z].worldWallId].point2 !=
           pvWalls[bunchWallsList[z]].worldWallId) ||
          (pvWalls[z].screenSpaceCoo[1][VEC_COL] >= pvWalls[bunchWallsList[z]].screenSpaceCoo[0][VEC_COL]))
      {
...
*/
static void
prepwall(int32_t z, walltype* wal)
{
...
static int32_t
getpalookup(int32_t davis, int32_t dashade)
{
  return (min(max(dashade + (davis >> 8), 0), numpalookups - 1));
}

static void
hline(int32_t xr, int32_t yp)
{
  int32_t xl, r, s;
...
static void
slowhline(int32_t xr, int32_t yp)
{
  int32_t xl, r;
  ...
  {
    mhline(globalbufplc, globaly1 * r + globalxpanning - asm1 * (xr - xl), (xr - xl) << 16, 0L, globalx2 * r + globalypanning - asm2 * (xr - xl), ylookup[yp] + xl + frameoffset);
    return;
  }
...
static void
ceilscan(int32_t x1, int32_t x2, int32_t sectnum)
{
...
static void
florscan(int32_t x1, int32_t x2, int32_t sectnum)
{
  int32_t i, j, ox, oy, x, y1, y2, twall, bwall;
  sectortype* sec;
 ...
 */
static void
wallscan(int32_t x1, int32_t x2, int16_t* uwal, int16_t* dwal, int32_t* swal, int32_t* lwal)
{
...
static void
maskwallscan(int32_t x1, int32_t x2, short* uwal, short* dwal, int32_t* swal, int32_t* lwal)
{
  ...
  for (; x <= x2 - 3; x += 4, p += 4)
  {
    ...
    {
      if (!(bad & 1))
        mvlineasm1(vince[0], palookupoffse[0], y2ve[0] - y1ve[0], vplce[0], (const uint8_t*)bufplce[0], ylookup[y1ve[0]] + p + 0);
      if (!(bad & 2))
        mvlineasm1(vince[1], palookupoffse[1], y2ve[1] - y1ve[1], vplce[1], (const uint8_t*)bufplce[1], ylookup[y1ve[1]] + p + 1);
      if (!(bad & 4))
        mvlineasm1(vince[2], palookupoffse[2], y2ve[2] - y1ve[2], vplce[2], (const uint8_t*)bufplce[2], ylookup[y1ve[2]] + p + 2);
      if (!(bad & 8))
        mvlineasm1(vince[3], palookupoffse[3], y2ve[3] - y1ve[3], vplce[3], (const uint8_t*)bufplce[3], ylookup[y1ve[3]] + p + 3);
      continue;
    }

    if (u4 > y1ve[0])
      vplce[0] = mvlineasm1(vince[0], palookupoffse[0], u4 - y1ve[0] - 1, vplce[0], (const uint8_t*)bufplce[0], ylookup[y1ve[0]] + p + 0);
    if (u4 > y1ve[1])
      vplce[1] = mvlineasm1(vince[1], palookupoffse[1], u4 - y1ve[1] - 1, vplce[1], (const uint8_t*)bufplce[1], ylookup[y1ve[1]] + p + 1);
    if (u4 > y1ve[2])
      vplce[2] = mvlineasm1(vince[2], palookupoffse[2], u4 - y1ve[2] - 1, vplce[2], (const uint8_t*)bufplce[2], ylookup[y1ve[2]] + p + 2);
    if (u4 > y1ve[3])
      vplce[3] = mvlineasm1(vince[3], palookupoffse[3], u4 - y1ve[3] - 1, vplce[3], (const uint8_t*)bufplce[3], ylookup[y1ve[3]] + p + 3);
    ...
    i = p + ylookup[d4 + 1];
    if (y2ve[0] > d4)
      mvlineasm1(vince[0], palookupoffse[0], y2ve[0] - d4 - 1, vplce[0], (const uint8_t*)bufplce[0], (uint8_t*)i + 0);
    if (y2ve[1] > d4)
      mvlineasm1(vince[1], palookupoffse[1], y2ve[1] - d4 - 1, vplce[1], (const uint8_t*)bufplce[1], (uint8_t*)i + 1);
    if (y2ve[2] > d4)
      mvlineasm1(vince[2], palookupoffse[2], y2ve[2] - d4 - 1, vplce[2], (const uint8_t*)bufplce[2], (uint8_t*)i + 2);
    if (y2ve[3] > d4)
      mvlineasm1(vince[3], palookupoffse[3], y2ve[3] - d4 - 1, vplce[3], (const uint8_t*)bufplce[3], (uint8_t*)i + 3);
  }
  for (; x <= x2; x++, p++)
  {
    y1ve[0] = max(uwal[x], startumost[x + windowx1] - windowy1);
...
static void
parascan(int32_t dax1, int32_t dax2, int32_t sectnum, uint8_t dastat, int32_t bunch)
{
  sectortype* sec;
...
static void
grouscan(int32_t dax1, int32_t dax2, int32_t sectnum, uint8_t dastat)
{
...
static int
owallmost(short* mostbuf, int32_t w, int32_t z)
{
...
static int
wallmost(short* mostbuf, int32_t w, int32_t sectnum, uint8_t dastat)
{
...
*/
int
wallfront(int32_t pvWallID1, int32_t pvWallID2)
{
  walltype* wal;
...
static int
bunchfront(int32_t firstBunchID, int32_t secondBunchID)
{
...
*/
void
drawrooms(int32_t daposx, int32_t daposy, int32_t daposz, short daang, int32_t dahoriz, short dacursectnum)
{
...
static int
spritewallfront(spritetype* s, int32_t w)
{
  walltype* wal;
  int32_t x1, y1;
...
static void
transmaskwallscan(int32_t x1, int32_t x2)
{
  int32_t x;
...
int
loadboard(char* filename, int32_t* daposx, int32_t* daposy, int32_t* daposz, short* daang, short* dacursectnum)
{
  int x;
...
static void
write8(int f, uint8_t val)
{
  write(f, &val, 1);
}

int
saveboard(char* filename, int32_t* daposx, int32_t* daposy, int32_t* daposz, short* daang, short* dacursectnum)
{
  int fil;
  int x;
...
static void
initfastcolorlookup(int32_t rscale, int32_t gscale, int32_t bscale)
{
  int32_t i, j, x, y, z;
  uint8_t* pal1;
...
int
setgamemode(int32_t daxdim, int32_t daydim)
{
...
static int
clippoly4(int32_t cx1, int32_t cy1, int32_t cx2, int32_t cy2)
{
      ...
      pvWalls[n].cameraSpaceCoo[0][VEC_X] =
        pvWalls[z].cameraSpaceCoo[1][VEC_X] + scale(t,
                                                    pvWalls[zz].cameraSpaceCoo[1][VEC_X] -
                                                      pvWalls[z].cameraSpaceCoo[1][VEC_X],
                                                    y2);
      n++;
    }
...
static void
dorotatesprite(int32_t sx, int32_t sy, int32_t z, short a, short picnum, int8_t dashade, uint8_t dapalnum, uint8_t dastat, int32_t cx1, int32_t cy1, int32_t cx2, int32_t cy2)
{
...
int
clipinsidebox(int32_t x, int32_t y, short wallnum, int32_t walldist)
{
  walltype* wal;
...
static int
clipinsideboxline(int32_t x, int32_t y, int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t walldist)
{
  int32_t r;
...
void
drawline256(int32_t x1, int32_t y1, int32_t x2, int32_t y2, uint8_t col)
{
...
int
inside(int32_t x, int32_t y, short sectnum)
{
  walltype* wal;
...
int
getangle(int32_t xvect, int32_t yvect)
{
  if ((xvect | yvect) == 0)
    return (0);
  if (xvect == 0)
...
static void
ceilspritehline(int32_t x2, int32_t y)
{
  int32_t x1, v, bx, by;
...
static void
ceilspritescan(int32_t x1, int32_t x2)
{
...
int
setsprite(short spritenum, int32_t newx, int32_t newy, int32_t newz)
{
  short tempsectnum;
...
int
nextsectorneighborz(short sectnum, int32_t thez, short topbottom, short direction)
{
  walltype* wal;
...
int
cansee(int32_t x1, int32_t y1, int32_t z1, short sect1, int32_t x2, int32_t y2, int32_t z2, short sect2)
{
  sectortype* sec;
  walltype *wal, *wal2;
...
int
lintersect(int32_t x1, int32_t y1, int32_t z1, int32_t x2, int32_t y2, int32_t z2, int32_t x3, int32_t y3, int32_t x4, int32_t y4, int32_t* intx, int32_t* inty, int32_t* intz)
...
int
rintersect(int32_t x1, int32_t y1, int32_t z1, int32_t vx, int32_t vy, int32_t vz, int32_t x3, int32_t y3, int32_t x4, int32_t y4, int32_t* intx, int32_t* inty, int32_t* intz)
...
int
hitscan(int32_t xs, int32_t ys, int32_t zs, short sectnum, int32_t vx, int32_t vy, int32_t vz, short* hitsect, short* hitwall, short* hitsprite, int32_t* hitx, int32_t* hity, int32_t* hitz, uint32_t cliptype)
{
  sectortype* sec;
  walltype *wal, *wal2;
  spritetype* spr;
...
int
neartag(int32_t xs, int32_t ys, int32_t zs, short sectnum, short ange, short* neartagsector, short* neartagwall, short* neartagsprite, int32_t* neartaghitdist, int32_t neartagrange, uint8_t tagsearch)
{
  walltype *wal, *wal2;
  spritetype* spr;
...
void
dragpoint(short pointhighlight, int32_t dax, int32_t day)
{
  short cnt, tempshort;
...
static void
keepaway(int32_t* x, int32_t* y, int32_t w)
{
...
static int
raytrace(int32_t x3, int32_t y3, int32_t* x4, int32_t* y4)
{
...
int
clipmove(int32_t* x, int32_t* y, int32_t* z, short* sectnum, int32_t xvect, int32_t yvect, int32_t walldist, int32_t ceildist, int32_t flordist, uint32_t cliptype)
{
  walltype *wal, *wal2;
  spritetype* spr;
  sectortype *sec, *sec2;
...
int
pushmove(int32_t* x, int32_t* y, int32_t* z, short* sectnum, int32_t walldist, int32_t ceildist, int32_t flordist, uint32_t cliptype)
{
  sectortype *sec, *sec2;
  walltype* wal;
 ...
 */
void
updatesector(int32_t x, int32_t y, short* lastKnownSector)
{
  walltype* wal;
  int32_t i, j;
...
void
rotatepoint(int32_t xpivot, int32_t ypivot, int32_t x, int32_t y, short daang, int32_t* x2, int32_t* y2)
{
  int32_t dacos, dasin;
...
void
draw2dgrid(int32_t posxe, int32_t posye, short ange, int32_t zoome, short gride)
{
...
void
draw2dscreen(int32_t posxe, int32_t posye, short ange, int32_t zoome, short gride)
{
  walltype* wal;
  ...
  for (i = numwalls - 1, wal = &wall[i]; i >= 0; i--, wal--)
  {
    if (editstatus == 0)
    {
      ...
      j = wal->nextwall;
      if ((j >= 0) && (i > j))
        if ((show2dwall[j >> 3] & pow2char[j & 7]) > 0)
          continue;
    }
    else
    {
 ...
 */
void
printext256(int32_t xpos, int32_t ypos, short col, short backcol, char* name, uint8_t fontsize)
{
...
void
getzrange(int32_t x, int32_t y, int32_t z, short sectnum, int32_t* ceilz, int32_t* ceilhit, int32_t* florz, int32_t* florhit, int32_t walldist, uint32_t cliptype)
{
  sectortype* sec;
  walltype *wal, *wal2;
  spritetype* spr;
...
void
setview(int32_t x1, int32_t y1, int32_t x2, int32_t y2)
{
  int32_t i;
...
void
setaspect(int32_t daxrange, int32_t daaspect)
{
...
void
rotatesprite(int32_t sx, int32_t sy, int32_t z, short a, short picnum, int8_t dashade, uint8_t dapalnum, uint8_t dastat, int32_t cx1, int32_t cy1, int32_t cx2, int32_t cy2)
{
  int32_t i;
...
static int
getclosestcol(int32_t r, int32_t g, int32_t b)
{
...
void
makepalookup(int32_t palnum, uint8_t* remapbuf, int8_t r, int8_t g, int8_t b, uint8_t dastat)
{
...
void
setbrightness(uint8_t dabrightness, uint8_t* dapal)
{
  int32_t i, k;
...
static int
clippoly(int32_t npoints, int32_t clipstat)
{
...
void
drawmapview(int32_t dax, int32_t day, int32_t zoome, int16_t ang)
{
  walltype* wal;
  sectortype* sec;
  spritetype* spr;
  ...
  for (s = 0, sec = &sector[s]; s < numsectors; s++, sec++)
    if (show2dsector[s >> 3] & pow2char[s & 7])
    {
      ...
      if ((globalorientation & 0x10) > 0)
        globalx1 = -globalx1, globaly1 = -globaly1, globalposx = -globalposx;
      if ((globalorientation & 0x20) > 0)
        globalx2 = -globalx2, globaly2 = -globaly2, globalposy = -globalposy;
...
void
plotpixel(int32_t x, int32_t y, uint8_t col)
{
...
void
preparemirror(int32_t dax, int32_t day, int32_t daz, short daang, int32_t dahoriz, short dawall, short dasector, int32_t* tposx, int32_t* tposy, short* tang)
{
...
int
getceilzofslope(short sectnum, int32_t dax, int32_t day)
{
  int32_t dx, dy, i, j;
  walltype* wal;
...
int
getflorzofslope(short sectnum, int32_t dax, int32_t day)
{
  int32_t dx, dy, i, j;
  walltype* wal;
 ...
 */
void
getzsofslope(short sectnum, int32_t dax, int32_t day, int32_t* ceilz, int32_t* florz)
{
  int32_t dx, dy, i, j;
  walltype *wal, *wal2;
  sectortype* sec;
...
void
alignceilslope(short dasect, int32_t x, int32_t y, int32_t z)
{
  int32_t i, dax, day;
  walltype* wal;
...
void
alignflorslope(short dasect, int32_t x, int32_t y, int32_t z)
{
  int32_t i, dax, day;
  walltype* wal;
