...
inline static void
__checkSdlSuccess(int result, char const* func, const char* file, int line)
{
  if (result != 0)
  {
    Error(EXIT_FAILURE,
...
inline static void
__checkSdlNotNull(void* handle, char const* msg, const char* file, int line)
{
  if (handle == NULL)
  {
    Error(EXIT_FAILURE,
...
inline static void
set_sdl_scancodes(uint32_t scancodes[], size_t size)
{
