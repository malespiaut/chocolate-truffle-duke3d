...
static void
go_to_new_vid_mode(int w, int h)
{
  if (window != NULL)
  {
...
static inline int
sdl_mouse_button_filter(SDL_MouseButtonEvent const* event)
{
  /*
...
static int
sdl_mouse_motion_filter(SDL_Event const* event)
{
  if (surface == NULL)
    return (0);
...
void
fullscreen_toggle_and_change_driver(void)
{
...
static int
root_sdl_event_filter(const SDL_Event* event)
{
  switch (event->type)
  {
    case SDL_KEYUP:
...
void
_platform_init(int argc, char** argv, const char* title, const char* iconName)
{
  int64_t timeElapsed;
...
int
_setgamemode(int32_t daxdim, int32_t daydim)
{
  int validated, i;
...
static int
get_dimensions_from_str(const char* str, int32_t* _w, int32_t* _h)
{
  char* xptr = NULL;
  char* ptr = NULL;
  int32_t w = -1;
...
static inline void
get_max_screen_res(int32_t* max_w, int32_t* max_h)
{
...
static inline void
add_user_defined_resolution(void)
{
  int32_t w;
  int32_t h;
...
static void
remove_vesa_mode(int index, const char* reason)
{
  int i;
...
static inline void
cull_duplicate_vesa_modes(void)
{
  int i;
  int j;
...
void
WritePaletteToFile(uint8_t* palette, const char* filename, int width, int height)
{
  uint8_t tga_header[18];
  uint8_t* buffer;
...
void
VBE_getPalette(int32_t start, int32_t num, uint8_t* palettebuffer)
{
...
void
VBE_setPalette(uint8_t* palettebuffer)
/*
 * (From Ken's docs:)
...
void
readmousebstatus(short* bstatus)
{
  if (bstatus)
...
void
_updateScreenRect(int32_t x, int32_t y, int32_t w, int32_t h)
{
...
uint8_t
readpixel(uint8_t* offset)
{
  return *offset;
}

void
drawpixel(uint8_t* location, uint8_t pixel)
{
  *location = pixel;
}
...
void
setcolor16(uint8_t col)
{
  drawpixel_color = col;
}

void
drawpixel16(int32_t offset)
{
  drawpixel((uint8_t*)surface->pixels + offset, drawpixel_color);
} /* drawpixel16 */

void
fillscreen16(int32_t offset, int32_t color, int32_t blocksize)
{
  Uint8* surface_end;
  Uint8* wanted_end;
  Uint8* pixels;
...
static inline void
DrawHorizontalRun(uint8_t** ScreenPtr, int XAdvance, int RunLength, uint8_t Color)
{
  int i;
...
static inline void
DrawVerticalRun(uint8_t** ScreenPtr, int XAdvance, int RunLength, uint8_t Color)
{
  int i;
  uint8_t* WorkingScreenPtr = *ScreenPtr;
...
void
drawline16(int32_t XStart, int32_t YStart, int32_t XEnd, int32_t YEnd, uint8_t Color)
{
  ...
  if (dx >= 0)
  {
    ...
    {
      if (dy)
        YStart += scale(0 - XStart, dy, dx);
      XStart = 0;
    }
    if (XEnd > 639)
    {
      if (dy)
        YEnd += scale(639 - XEnd, dy, dx);
      XEnd = 639;
    }
  }
  else
  {
    if ((XEnd > 639) || (XStart < 0))
      return;
    if (XEnd < 0)
    {
      if (dy)
        YEnd += scale(0 - XEnd, dy, dx);
      XEnd = 0;
    }
    if (XStart > 639)
    {
      if (dy)
        YStart += scale(639 - XStart, dy, dx);
      XStart = 639;
    }
  }
  if (dy >= 0)
  {
    if ((YStart >= ydim16) || (YEnd < 0))
      return;
    if (YStart < 0)
    {
      if (dx)
        XStart += scale(0 - YStart, dx, dy);
      YStart = 0;
    }
    if (YEnd >= ydim16)
    {
      if (dx)
        XEnd += scale(ydim16 - 1 - YEnd, dx, dy);
      YEnd = ydim16 - 1;
    }
  }
  else
  {
    if ((YEnd >= ydim16) || (YStart < 0))
      return;
    if (YEnd < 0)
    {
      if (dx)
        XEnd += scale(0 - YEnd, dx, dy);
      YEnd = 0;
    }
    if (YStart >= ydim16)
    {
      if (dx)
        XStart += scale(ydim16 - 1 - YStart, dx, dy);
      YStart = ydim16 - 1;
    }
  }
