...
neton(void)
{
  ...
  if ((simulateint(0x2f, (long)0x7a00, 0L, 0L, 0L, 0L, 0L) & 255) != 255)
    return (-1);
  if (*(long*)(0x7a << 2) == 0)
  {
#ifdef PLATFORM_DOS
  ...
  if ((simulateint(0x7a, (long)0xff, 0L, 0L, (long)socket, 0L, 0L) & 255) != 0)
    return (-2); // Opensocket
...
comon()
{
  ...
  *comport = *ptr;
  if (*comport == 0)
  {
    ...
    if ((inp((*comport) + 5) & 0x60) != 0x60)
    {
      *comport = 0;
      return (-1);
    }
  }
  if ((comspeed <= 0) || (comspeed > 115200))
    return (-1);
...
comsend(char ch)
{
  if (ch == ESC1)
  {
    outbuf[*outbufend] = ESC1;
    *outbufend = (((*outbufend) + 1) & (COMBUFSIZ - 1));
    ch = 128;
  }
  else if (ch == ESC2)
  {
...
netinitconnection(long newconnectnum, char* newcompaddr)
{
  ...
  while (connectnum[newindex] != 0x7fffffff)
  {
    ...
    if (newindex >= MAXPLAYERS)
      return (-1); // Out of space! (more than 16 players)
  }
  ...
  for (i = 1; i < MAXPLAYERS; i++)
    for (j = 0; j < i; j++)
      if (connectnum[i] < connectnum[j])
      {
        ...
        for (k = 0; k < 12; k++)
          tempchar = compaddr[i][k], compaddr[i][k] = compaddr[j][k], compaddr[j][k] = tempchar;
      }
...
netuninitconnection(short goneindex)
{
  ...
  connectnum[goneindex] = 0x7fffffff;
  numplayers--;
...
void
sendpacket(short otherconnectindex, unsigned char* bufptr, short messleng)
{
  long i, j, k, l;
  ...
  else
  {
    ...
    while ((i > 0) && (ecbput[8] != 0))
      i--;

    messout[0] = myconnectindex;
    j = 1;
    ...
    if (otherconnectindex < 0)
    {
      for (k = connecthead; k >= 0; k = connectpoint2[k])
        if (k != myconnectindex)
        {
          ...
          for (i = 0; i < messleng; i++)
            omessout[k][omessnum[k]][i] = bufptr[i];
        }
    }
    else
    {
      ...
      for (i = 0; i < messleng; i++)
        omessout[otherconnectindex][omessnum[otherconnectindex]][i] = bufptr[i];
    }
  ...
  if (multioption < 5)
  {
    *otherconnectindex = (myconnectindex ^ 1);
    ...
    while (*inbufplc != *inbufend)
    {
      ...
      if (i != ESC2)
      {
        ...
        if (comescape != 0)
        {
          ...
          if (syncbufleng < MAXIPXSIZ)
          {
            if (i == 128)
            {
              syncbuf[syncbufleng++] = ESC1;
              continue;
            }
            if (i == 129)
            {
              syncbuf[syncbufleng++] = ESC2;
              continue;
            }
          }
        }
        if (syncbufleng < MAXIPXSIZ)
          syncbuf[syncbufleng++] = i;
        continue;
      }
      ...
      if (((syncbuf[0] & 128) >> 7) != (messleng & 1))
        bad |= 2; // messleng error
      for (i = 0; i < messleng; i++)
        bufptr[i] = syncbuf[i + 1];
      if ((comrateoption & 15) > 0)
      {
        i = getcrc(bufptr, messleng);
  ...
  else
  {
    ...
    *otherconnectindex = getmess[k++];
    ...
    for (totbad = 0; totbad < NETBACKPACKETS; totbad++) // Number of sub-packets per packet
    {
      ...
      if (*otherconnectindex == myconnectindex)
        return (0);
