...
void
gamenumber(int32_t x, int32_t y, int32_t n, uint8_t s)
{
  char b[10];
...
char recbuf[80];
void
allowtimetocorrecterrorswhenquitting(void)
{
...
char*
grpVersion2char(uint8_t grp_to_identify)
{
  char* id;
    ...
    switch (packbuf[0])
    {
      ...
      case 0: //[0] (receive master sync buffer)
        ...
        for (i = connecthead; i >= 0; i = connectpoint2[i])
        {
          ...
          if (l & 8)
            nsyn[i].bits = ((nsyn[i].bits & 0xffffff00) | ((int32_t)packbuf[j++]));
          if (l & 16)
            nsyn[i].bits = ((nsyn[i].bits & 0xffff00ff) | ((int32_t)packbuf[j++]) << 8);
          if (l & 32)
            nsyn[i].bits = ((nsyn[i].bits & 0xff00ffff) | ((int32_t)packbuf[j++]) << 16);
          if (l & 64)
            nsyn[i].bits = ((nsyn[i].bits & 0x00ffffff) | ((int32_t)packbuf[j++]) << 24);
          if (l & 128)
      ...
      case 1: //[1] (receive slave sync buffer)
        ...
        if (k & 8)
          nsyn[other].bits = ((nsyn[other].bits & 0xffffff00) | ((int32_t)packbuf[j++]));
        if (k & 16)
          nsyn[other].bits = ((nsyn[other].bits & 0xffff00ff) | ((int32_t)packbuf[j++]) << 8);
        if (k & 32)
          nsyn[other].bits = ((nsyn[other].bits & 0xff00ffff) | ((int32_t)packbuf[j++]) << 16);
        if (k & 64)
          nsyn[other].bits = ((nsyn[other].bits & 0x00ffffff) | ((int32_t)packbuf[j++]) << 24);
        if (k & 128)
...
dispVersion(void)
{
  ...
  rotatesprite(0 << 16, 0 << 16, 65536l << 5, 0, BLANK, 8, 0, 1 + 2 + 8 + 16 + 64, scale(0, xdim, 320), scale(26, ydim, 200), scale(320 - 0, xdim, 320) - 1, scale(200 - ((ud.multimode > 4) ? (161 - 1 * 7) - stepy : (161 - 1 * 7)), ydim, 200) - 1);
...
void
invennum(int32_t x, int32_t y, uint8_t num1, uint8_t ha, uint8_t sbits)
{
  char dabuf[80] = {0};
...
void
weaponnum999(uint8_t ind, int32_t x, int32_t y, int32_t num1, int32_t num2, uint8_t ha)
{
  char dabuf[80] = {0};
  ...
  else if (num1 > 9)
  {
    ...
    rotatesprite((x + 8) << 16, y << 16, 65536L, 0, THREEBYFIVE + dabuf[0] - '0', ha, 0, 10 + 128, 0, 0, xdim - 1, ydim - 1);
  ...
  else if (num2 > 9)
  {
    ...
    rotatesprite((x + 25) << 16, y << 16, 65536L, 0, THREEBYFIVE + dabuf[0] - '0', ha, 0, 10 + 128, 0, 0, xdim - 1, ydim - 1);
}
...
weapon_amounts(struct player_struct* p, int32_t x, int32_t y, int32_t u)
{
  ...
  if (u & 4)
  {
    ...
    weaponnum999(PISTOL_WEAPON, x, y, p->ammo_amount[PISTOL_WEAPON], max_ammo_amount[PISTOL_WEAPON], 12 - 20 * (cw == PISTOL_WEAPON));
  }
  if (u & 8)
  {
    if (u != 0xffffffff)
      patchstatusbar(96, 184, 96 + 12, 184 + 6);
    weaponnum999(SHOTGUN_WEAPON, x, y + 6, p->ammo_amount[SHOTGUN_WEAPON], max_ammo_amount[SHOTGUN_WEAPON], (!p->gotweapon[SHOTGUN_WEAPON] * 9) + 12 - 18 * (cw == SHOTGUN_WEAPON));
  }
  if (u & 16)
  {
    if (u != 0xffffffff)
      patchstatusbar(96, 190, 96 + 12, 190 + 6);
    weaponnum999(CHAINGUN_WEAPON, x, y + 12, p->ammo_amount[CHAINGUN_WEAPON], max_ammo_amount[CHAINGUN_WEAPON], (!p->gotweapon[CHAINGUN_WEAPON] * 9) + 12 - 18 * (cw == CHAINGUN_WEAPON));
  }
  if (u & 32)
  {
    if (u != 0xffffffff)
      patchstatusbar(135, 178, 135 + 8, 178 + 6);
    weaponnum(RPG_WEAPON, x + 39, y, p->ammo_amount[RPG_WEAPON], max_ammo_amount[RPG_WEAPON], (!p->gotweapon[RPG_WEAPON] * 9) + 12 - 19 * (cw == RPG_WEAPON));
  }
  if (u & 64)
  {
    if (u != 0xffffffff)
      patchstatusbar(135, 184, 135 + 8, 184 + 6);
    weaponnum(HANDBOMB_WEAPON, x + 39, y + 6, p->ammo_amount[HANDBOMB_WEAPON], max_ammo_amount[HANDBOMB_WEAPON], (((!p->ammo_amount[HANDBOMB_WEAPON]) | (!p->gotweapon[HANDBOMB_WEAPON])) * 9) + 12 - 19 * ((cw == HANDBOMB_WEAPON) || (cw == HANDREMOTE_WEAPON)));
  }
  if (u & 128)
  {
    if (u != 0xffffffff)
    ...
    if (VOLUMEONE)
    {
      orderweaponnum(SHRINKER_WEAPON, x + 39, y + 12, p->ammo_amount[SHRINKER_WEAPON], max_ammo_amount[SHRINKER_WEAPON], (!p->gotweapon[SHRINKER_WEAPON] * 9) + 12 - 18 * (cw == SHRINKER_WEAPON));
    }
    else
    {
      if (p->subweapon & (1 << GROW_WEAPON))
        weaponnum(SHRINKER_WEAPON, x + 39, y + 12, p->ammo_amount[GROW_WEAPON], max_ammo_amount[GROW_WEAPON], (!p->gotweapon[GROW_WEAPON] * 9) + 12 - 18 * (cw == GROW_WEAPON));
      else
        weaponnum(SHRINKER_WEAPON, x + 39, y + 12, p->ammo_amount[SHRINKER_WEAPON], max_ammo_amount[SHRINKER_WEAPON], (!p->gotweapon[SHRINKER_WEAPON] * 9) + 12 - 18 * (cw == SHRINKER_WEAPON));
    }
  }
  if (u & 256)
  {
    ...
    if (VOLUMEONE)
    {
      orderweaponnum(DEVISTATOR_WEAPON, x + 70, y, p->ammo_amount[DEVISTATOR_WEAPON], max_ammo_amount[DEVISTATOR_WEAPON], (!p->gotweapon[DEVISTATOR_WEAPON] * 9) + 12 - 18 * (cw == DEVISTATOR_WEAPON));
    }
    else
    {
      weaponnum(DEVISTATOR_WEAPON, x + 70, y, p->ammo_amount[DEVISTATOR_WEAPON], max_ammo_amount[DEVISTATOR_WEAPON], (!p->gotweapon[DEVISTATOR_WEAPON] * 9) + 12 - 18 * (cw == DEVISTATOR_WEAPON));
    }
  }
  if (u & 512)
  {
    ...
    if (VOLUMEONE)
    {
      orderweaponnum(TRIPBOMB_WEAPON, x + 70, y + 6, p->ammo_amount[TRIPBOMB_WEAPON], max_ammo_amount[TRIPBOMB_WEAPON], (!p->gotweapon[TRIPBOMB_WEAPON] * 9) + 12 - 18 * (cw == TRIPBOMB_WEAPON));
    }
    else
    {
      weaponnum(TRIPBOMB_WEAPON, x + 70, y + 6, p->ammo_amount[TRIPBOMB_WEAPON], max_ammo_amount[TRIPBOMB_WEAPON], (!p->gotweapon[TRIPBOMB_WEAPON] * 9) + 12 - 18 * (cw == TRIPBOMB_WEAPON));
    }
  }
  ...
  if (u & 65536L)
  {
    ...
    if (VOLUMEONE)
    {
      orderweaponnum(-1, x + 70, y + 12, p->ammo_amount[FREEZE_WEAPON], max_ammo_amount[FREEZE_WEAPON], (!p->gotweapon[FREEZE_WEAPON] * 9) + 12 - 18 * (cw == FREEZE_WEAPON));
    }
    else
    {
      weaponnum(-1, x + 70, y + 12, p->ammo_amount[FREEZE_WEAPON], max_ammo_amount[FREEZE_WEAPON], (!p->gotweapon[FREEZE_WEAPON] * 9) + 12 - 18 * (cw == FREEZE_WEAPON));
    }
  }
}
...
displayfragbar(void)
{
  ...
  if (j >= 4)
    rotatesprite(319, (8) << 16, 65600L, 0, FRAGBAR, 0, 0, 10 + 16 + 64 + 128, 0, 0, xdim - 1, ydim - 1);
  if (j >= 8)
    rotatesprite(319, (16) << 16, 65600L, 0, FRAGBAR, 0, 0, 10 + 16 + 64 + 128, 0, 0, xdim - 1, ydim - 1);
  if (j >= 12)
    rotatesprite(319, (24) << 16, 65600L, 0, FRAGBAR, 0, 0, 10 + 16 + 64 + 128, 0, 0, xdim - 1, ydim - 1);
...
coolgaugetext(int16_t snum)
{
  ...
  else
  {
    if (u & 16384)
    {
      ...
      if (p->got_access & 4)
        rotatesprite(275 << 16, 182 << 16, 65536L, 0, ACCESS_ICON, 0, 23, 10 + 16 + 128, 0, 0, xdim - 1, ydim - 1);
      if (p->got_access & 2)
        rotatesprite(288 << 16, 182 << 16, 65536L, 0, ACCESS_ICON, 0, 21, 10 + 16 + 128, 0, 0, xdim - 1, ydim - 1);
      if (p->got_access & 1)
        rotatesprite(281 << 16, 189 << 16, 65536L, 0, ACCESS_ICON, 0, 0, 10 + 16 + 128, 0, 0, xdim - 1, ydim - 1);
    }
  }
...
operatefta(void)
{
  ...
  for (i = 0; i < MAXUSERQUOTES; i++)
  {
    ...
    if (k > 4)
      ...
      gametext(320 >> 1, j, user_quote[i], 0, 2 + 8 + 16 + 1);
    else
      gametext(320 >> 1, j, user_quote[i], 0, 2 + 8 + 16 + 1 + 32);
    j -= 8;
  }
...
void
displayrest(int32_t smoothratio)
{
  int32_t a, i, j;
...
void
updatesectorz(int32_t x, int32_t y, int32_t z, short* sectnum)
{
  walltype* wal;
  int32_t i, j, cz, fz;
...
nonsharedkeys(void)
{
  ...
  if (ps[myconnectindex].cheat_phase == 1 || ps[myconnectindex].gm & (MODE_MENU | MODE_TYPE))
    return;
  ...
  if (KB_KeyPressed(sc_F11))
  {
    ...
    if (SHIFTS_IS_PRESSED)
      ud.brightness -= 8; // Keyboard step must be 8, as the brightness cursor step.
    else
      ud.brightness += 8;
    ...
    if (ud.brightness < 40)
      FTA(29 + (ud.brightness >> 3), &ps[myconnectindex], 1);
    else if (ud.brightness < 80)
      FTA(96 + (ud.brightness >> 3) - 5, &ps[myconnectindex], 1);
  }
}

void
comlinehelp(char** argv)
{
...
getnames(void)
{
  ...
  if (numplayers > 1)
  {
    ...
    for (i = connecthead; i >= 0; i = connectpoint2[i])
      if (i != myconnectindex)
        sendpacket(i, (uint8_t*)buf, 1 + sizeof(groupefil_crc32) + sizeof(ud.conSize[0]) + sizeof(ud.conCRC[0]) + sizeof(ud.exeCRC[0]));
      ...
      for (i = connecthead; i >= 0; i = connectpoint2[i])
      {
        for (j = 0; j < MAXGROUPFILES && ud.groupefil_crc32[i][j]; j++)
        {
          if (j)
            ...
            printf("Player [%-10s] GRP (base)   : %s CRC=%X\n", ud.user_name[i], grpVersion2char_from_crc(ud.groupefil_crc32[i][j]), ud.groupefil_crc32[i][j]);
        }
...
void
findGRPToUse(uint8_t* groupfilefullpath)
{
...
#include <dirent.h>
void
findGRPToUse(char* groupfilefullpath)
{
...
uint8_t
opendemoread(uint8_t which_demo) // 0 = mine
{
  char d[] = "demo_.dmo";
  char* fname = d;
  uint8_t ver;
      ...
      if (groupefil_crc32_from_demo[i] != groupefil_crc32[i])
      {
        for (j = 0; j <= i; j++)
        {
          printf("You have GRP #%d:  %s (CRC32=%X)\n"
                 ...
                 j,
                 grpVersion2char_from_crc(groupefil_crc32[j]),
                 groupefil_crc32[j],
...
dobonus(uint8_t bonusonly)
{
  ...
  int32_t breathe[] =
    {
      0, 30, VICTORY1 + 1, 176, 59, 30, 60, VICTORY1 + 2, 176, 59, 60, 90, VICTORY1 + 1, 176, 59, 90, 120, 0, 176, 59};

  int32_t bossmove[] =
    {
      0, 120, VICTORY1 + 3, 86, 59, 220, 260, VICTORY1 + 4, 86, 59, 260, 290, VICTORY1 + 5, 86, 59, 290, 320, VICTORY1 + 6, 86, 59, 320, 350, VICTORY1 + 7, 86, 59, 350, 380, VICTORY1 + 8, 86, 59};
    ...
    if (ps[myconnectindex].gm & MODE_EOL)
    {
      ...
      if (totalclock > (60 * 3))
      {
        ...
        if (totalclock > (60 * 4))
        {
          ...
          sprintf(text, "%02d:%02d", (ps[myconnectindex].player_par / (26 * 60)) % 60, (ps[myconnectindex].player_par / 26) % 60);
          gametext((320 >> 2) + 71, 60 + 9, text, 0, 2 + 8 + 16);

          sprintf(text, "%02d:%02d", (partime[ud.volume_number * 11 + ud.last_level - 1] / (26 * 60)) % 60, (partime[ud.volume_number * 11 + ud.last_level - 1] / 26) % 60);
          gametext((320 >> 2) + 71, 69 + 9, text, 0, 2 + 8 + 16);

          sprintf(text, "%02d:%02d", (designertime[ud.volume_number * 11 + ud.last_level - 1] / (26 * 60)) % 60, (designertime[ud.volume_number * 11 + ud.last_level - 1] / 26) % 60);
        ...
        if (totalclock > (60 * 7))
        {
          ...
          else
          {
            if ((ps[myconnectindex].max_actors_killed - ps[myconnectindex].actors_killed) < 0)
              ...
              sprintf(text, "%-3d", ps[myconnectindex].max_actors_killed - ps[myconnectindex].actors_killed);
            gametext((320 >> 2) + 70, 99 + 4 + 9, text, 0, 2 + 8 + 16);
          }
        }
      }
...
takescreenshot(void)
{
  ...
  sprintf(text, "Chocolate DukeNukem3D(v%d.%d) %.4d.%.2d.%.2d %.2dh%.2dm%.2ds", CHOCOLATE_DUKE_REV_X, CHOCOLATE_DUKE_REV_DOT_Y, tmHMS->tm_year + 1900, tmHMS->tm_mon + 1, tmHMS->tm_mday, tmHMS->tm_hour, tmHMS->tm_min, tmHMS->tm_sec);
