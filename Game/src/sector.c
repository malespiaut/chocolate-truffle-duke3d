...
short
callsound(short sn, short whatsprite)
{
  short i;
...
short
check_activator_motion(short lotag)
{
  short i, j;
  spritetype* s;
...
int32_t
ldist(spritetype* s1, spritetype* s2)
{
  int32_t vx, vy;
  vx = s1->x - s2->x;
  vy = s1->y - s2->y;
...
int32_t
dist(spritetype* s1, spritetype* s2)
{
  int32_t vx, vy, vz;
  vx = s1->x - s2->x;
  vy = s1->y - s2->y;
...
short
findplayer(spritetype* s, int32_t* d)
{
...
short
findotherplayer(short p, int32_t* d)
{
  short j, closest_player;
  int32_t x, closest;
...
uint8_t
activatewarpelevators(short s, short d) // Parm = sectoreffectornum
{
  short i, sn;
...
void
operatesectors(short sn, short ii)
{
...
operaterespawns(short low)
{
  ...
  while (i >= 0)
  {
    ...
    if (SLT == low)
      ...
      {
        case RESPAWN:
          if (badguypic(SHT) && ud.monsters_off)
            break;
...
void
operateactivators(short low, short snum)
{
  short i, j, k, *p;
  walltype* wal;
...
void
operatemasterswitches(short low)
{
  short i;
...
void
operateforcefields(short s, short low)
{
  short i, p;
...
uint8_t
checkhitswitch(short snum, int32_t w, uint8_t switchtype)
{
  uint8_t switchpal;
...
void
activatebysector(short sect, short j)
{
  short i, didit;
...
void
breakwall(short newpn, short spr, short dawallnum)
{
...
void
checkhitwall(short spr, short dawallnum, int32_t x, int32_t y, int32_t z, short atwith)
{
          ...
          case BIGFORCE:
            ...
            else
            {
              if (atwith == CHAINGUN)
                ...
                i = EGS(sn, x, y, z, FORCERIPPLE, -127, 32, 32, 0, 0, 0, spr, 5);
            }

            CS |= 18 + 128;
            SA = getangle(wal->x - wall[wal->point2].x,
                          wal->y - wall[wal->point2].y) -
                 512;
...
void
checkplayerhurt(struct player_struct* p, short j)
{
  if ((j & 49152) == 49152)
  {
    j &= (MAXSPRITES - 1);
  ...
  if (p->hurt_delay > 0)
    ...
    switch (wall[j].overpicnum)
    {
      ...
      case W_FORCEFIELD + 2:
        ...
        checkhitwall(p->i, j, p->posx + (sintable[(p->ang + 512) & 2047] >> 9), p->posy + (sintable[p->ang & 2047] >> 9), p->posz, -1);
...
void
checkhitsprite(short i, short sn)
{
  short j, k, p;
  spritetype* s;
  ...
  switch (PN)
  {
    ...
    case ANTENNA:
      ...
      {
        for (j = 0; j < 15; j++)
          EGS(SECT, SX, SY, sector[SECT].floorz - (12 << 8) - (j << 9), SCRAP1 + (TRAND & 15), -8, 64, 64, TRAND & 2047, (TRAND & 127) + 64, -(TRAND & 511) - 256, i, 5);
        spawn(i, EXPLOSION2);
        deletesprite(i);
      }
      break;
      ...
      if ((p->weapon_pos == 0 || (p->holster_weapon && p->weapon_pos == -9)))
      {
        if (j == 10 || j == 11)
        {
          ...
          while ((k >= 0 && k < 10) || (k == GROW_WEAPON && (p->subweapon & (1 << GROW_WEAPON))))
          {
            ...
            else if (k == GROW_WEAPON && p->ammo_amount[GROW_WEAPON] == 0 && p->gotweapon[SHRINKER_WEAPON] && p->ammo_amount[SHRINKER_WEAPON] > 0)
            {
              j = SHRINKER_WEAPON;
              p->subweapon &= ~(1 << GROW_WEAPON);
              break;
            }
            else if (k == SHRINKER_WEAPON && p->ammo_amount[SHRINKER_WEAPON] == 0 && p->gotweapon[SHRINKER_WEAPON] && p->ammo_amount[GROW_WEAPON] > 0)
            {
              j = GROW_WEAPON;
              p->subweapon |= (1 << GROW_WEAPON);
              break;
        ...
        else if (j < MAX_WEAPONS && p->gotweapon[j] && p->curr_weapon != j)
          switch (j)
          {
            case KNEE_WEAPON:
              addweapon(p, KNEE_WEAPON);
