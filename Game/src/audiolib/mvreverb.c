...
check_buffer()
{
  ...
  if (!delay || new_delay != delay)
  {
    ...
    IN_COEF_R = -2.;
    if (reverbBuffer)
      reverbBuffer = (double*)realloc(reverbBuffer, new_delay * sizeof(double));
    else
      reverbBuffer = (double*)malloc(new_delay * sizeof(double));
...
double
g_buffer(int iOff, double* ptr) // get_buffer content helper: takes care about wraps
{
...
void
s_buffer(int iOff, double iVal, double* ptr) // set_buffer content helper: takes care about wraps and clipping
{
  int correctDelay = delay;
  if (!correctDelay)
  {
...
void
s_buffer1(int iOff, double iVal, double* ptr) // set_buffer (+1 sample) content helper: takes care about wraps and clipping
{
  int correctDelay = delay;
  if (!correctDelay)
  {
...
double
MixREVERBLeft(double INPUT_SAMPLE_L, double INPUT_SAMPLE_R, double* ptr)
{
...
void
MV_16BitDownmix(uint8_t* dest, int count)
{
  int i;
...
void
MV_8BitDownmix(uint8_t* dest, int count)
{
  int i;
...
void
MV_16BitReverbFast(const uint8_t* src, uint8_t* dest, int count, int shift)
{
  int i;
...
void
MV_8BitReverbFast(const int8_t* src, int8_t* dest, int count, int shift)
{
  int i;
