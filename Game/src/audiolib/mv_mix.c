...
static int
MV_cubic16(const short* src, int position, int rate)
{
  int temp, hpos = position >> 16;
...
static int
MV_cubic8to16(const unsigned char* src, int position, int rate)
{
  int temp, hpos = position >> 16;
...
void
MV_Mix8BitMono(uint32_t position, uint32_t rate, const char* start, uint32_t length)
{
...
void
MV_Mix8BitStereo(uint32_t position,
                 uint32_t rate,
                 const char* start,
                 uint32_t length)
{
  ...
  for (i = 0; i < length; i++)
  {
    ...
    if (dr < -32768)
      dest[MV_RightChannelOffset] = 0;
    else if (dr > 32767)
      dest[MV_RightChannelOffset] = 255;
    else
      dest[MV_RightChannelOffset] = (dl >> 8) + 128;
...
void
MV_Mix16BitMono(uint32_t position,
                uint32_t rate,
                const char* start,
                uint32_t length)
{
...
void
MV_Mix16BitStereo(uint32_t position,
                  uint32_t rate,
                  const char* start,
                  uint32_t length)
{
  ...
  for (i = 0; i < length; i++)
  {
    ...
    if (dr < -32768)
      dest[MV_RightChannelOffset / 2] = -32768;
    else if (dr > 32767)
      dest[MV_RightChannelOffset / 2] = 32767;
    else
      dest[MV_RightChannelOffset / 2] = dr;
...
void
MV_Mix8BitMono16(uint32_t position, uint32_t rate, const char* start, uint32_t length)
{
  const short* src;
  unsigned char* dest;
  unsigned int i;
...
void
MV_Mix8BitStereo16(uint32_t position,
                   uint32_t rate,
                   const char* start,
                   uint32_t length)
{
...
void
MV_Mix16BitMono16(uint32_t position,
                  uint32_t rate,
                  const char* start,
                  uint32_t length)
{
...
void
MV_Mix16BitStereo16(uint32_t position,
                    uint32_t rate,
                    const char* start,
                    uint32_t length)
{
  ...
  for (i = 0; i < length; i++)
  {
    ...
    if (dr < -32768)
      ...
      dest[MV_RightChannelOffset / 2] = dl;
...
void
MV_MixFPMono8(uint32_t position,
              uint32_t rate,
              const char* start,
              uint32_t length)
{
...
void
MV_MixFPStereo8(uint32_t position,
                uint32_t rate,
                const uint8_t* start,
                uint32_t length)
{
...
void
MV_MixFPMono16(uint32_t position,
               uint32_t rate,
               const uint8_t* start,
               uint32_t length)
{
...
void
MV_MixFPStereo16(uint32_t position,
                 uint32_t rate,
                 const uint8_t* start,
                 uint32_t length)
{
