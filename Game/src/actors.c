...
void
setinterpolation(int32_t* posptr)
{
  int32_t i;
...
void
stopinterpolation(int32_t* posptr)
{
  int32_t i;
...
void
dointerpolations(int32_t smoothratio) // Stick at beginning of drawscreen
{
...
void
addammo(int16_t weapon, struct player_struct* p, int16_t amount)
{
...
void
checkavailinven(struct player_struct* p)
{
    ...
    switch (s->picnum)
    {
      ...
      case STRIPEBALL:
        if (s->xvel)
        {
          ...
          j = clipmove(&s->x, &s->y, &s->z, &s->sectnum, (((s->xvel * (sintable[(s->ang + 512) & 2047])) >> 14) * TICSPERFRAME) << 11, (((s->xvel * (sintable[s->ang & 2047])) >> 14) * TICSPERFRAME) << 11, 24L, (4 << 8), (4 << 8), CLIPMASK1);
    ...
    switch (st)
    {
      ...
      case 9:
        ...
        if (hittype[i].temp_data[4])
        {
          ...
          j = getanimationgoal(&sc->ceilingz);
