...
p_console_current_view = p_console_current_view->next;
      ...
      */
      ...
      else if (console_used_command_list_current != NULL)
      {
        sprintf(dirty_buffer, "%s", console_used_command_list_current->text);
        CONSOLE_RecalculateDirtyBuffer();
      }
...
CONSOLE_Render()
{
  ...
  if (nConsole_Active)
  {
    ...
    for (i = 0; i < MAX_CONSOLE_VIEWABLE; i++)
    {
      ...
      minitext(5, (8 * (9 - i)) + iYOffset, pElement->text, g_CV_console_text_color, 10 + 16);
  ...
  else if (g_CV_num_console_lines > 0)
  {
    ...
    for (i = 0; i < g_CV_num_console_lines; i++)
    {
      ...
      minitext(5, (8 * ((g_CV_num_console_lines - 1) - i)) + iYOffset, pElement->text, g_CV_console_text_color, 10 + 16);
...
void
CONSOLE_ParseCommand(char* command)
{
  char* cvar;
  char* token;
  int i, numCvars;
...
void
CONSOLE_InsertUsedCommand(const char* szUsedCommand)
{
...
void
CONSOLE_Printf(const char* newmsg, ...)
{
  CONSOLEELEMENT* pElement;
  va_list argptr;
...
char*
CONSOLE_GetArgv(unsigned int var)
{
  return argv[var];
}
