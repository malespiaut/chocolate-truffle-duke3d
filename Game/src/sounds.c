...
int
USRHOOKS_GetMem(void** ptr, uint32_t size)
{
  *ptr = malloc(size);
...
int
xyzsound(short num, short i, int32_t x, int32_t y, int32_t z)
{
  ...
  if (soundm[num] & 1)
  {
    ...
    if (*Sound[num].ptr == 'C')
      voice = FX_PlayLoopedVOC(Sound[num].ptr, start, start + soundsiz[num], pitch, sndist >> 6, sndist >> 6, 0, soundpr[num], num);
    else
      voice = FX_PlayLoopedWAV(Sound[num].ptr, start, start + soundsiz[num], pitch, sndist >> 6, sndist >> 6, 0, soundpr[num], num);
  }
  else
  {
    if (*Sound[num].ptr == 'C')
...
sound(short num)
{
  ...
  if (SoundToggle == 0)
    return;
  if (VoiceToggle == 0 && (soundm[num] & 4))
    return;
  if ((soundm[num] & 8) && ud.lockout)
    return;
  if (FX_VoiceAvailable(soundpr[num]) == 0)
    return;
  ...
  if (soundm[num] & 1)
  {
    if (*Sound[num].ptr == 'C')
    {
      ...
      voice = FX_PlayLoopedVOC(Sound[num].ptr, start, start + soundsiz[num], pitch, LOUDESTVOLUME, LOUDESTVOLUME, LOUDESTVOLUME, soundpr[num], num);
    }
    else
    {
      start = (int32_t) * (uint16_t*)(Sound[num].ptr + 0x14);
      voice = FX_PlayLoopedWAV(Sound[num].ptr, start, start + soundsiz[num], pitch, LOUDESTVOLUME, LOUDESTVOLUME, LOUDESTVOLUME, soundpr[num], num);
    }
  }
  else
  {
...
int
spritesound(uint16_t num, short i)
{
  if (num >= NUM_SOUNDS)
    return -1;
...
void
stopenvsound(short num, short i)
{
  short j, k;
...
pan3dsound(void)
{
  ...
  for (j = 0; j < NUM_SOUNDS; j++)
    for (k = 0; k < Sound[j].num; k++)
    {
      ...
      if (Sound[j].ptr == 0 && loadsound(j) == 0)
        continue;
      if (soundm[j] & 16)
        sndist = 0;
