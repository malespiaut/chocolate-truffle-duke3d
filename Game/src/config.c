...
readsavenames(void)
{
  ...
  for (i = 0; i < 10; i++)
  {
    ...
    if ((fil = fopen(fullpathsavefilename, "rb")) == NULL)
      continue;
...
CONFIG_WriteSetup(void)
{
  ...
  for (i = 0; i < MAXMOUSEBUTTONS; i++)
  {
    ...
    SCRIPT_PutString(scripthandle, "Controls", (char*)tempbuf, (MouseMapping[i] != -1) ? CONFIG_FunctionNumToName(MouseMapping[i]) : "");
  }

  for (i = 0; i < MAXMOUSEAXES * 2; i++)
  {
    sprintf((char*)tempbuf, "MouseDigitalAxes%d_%d", i >> 1, i & 1);
    SCRIPT_PutString(scripthandle, "Controls", (char*)tempbuf, (MouseDigitalAxeMapping[i >> 1][i & 1] != -1) ? CONFIG_FunctionNumToName(MouseDigitalAxeMapping[i >> 1][i & 1]) : "");
  }
