...
static void
SCRIPT_freenode(scriptnode_t* node)
{
  assert(node != NULL);
...
static void
SCRIPT_writenode(scriptnode_t* node, FILE* fp)
{
  switch (node->type)
  {
...
static void
SCRIPT_recursivewrite(scriptnode_t* node, FILE* fp)
{
  if (node == NULL)
    return;
...
static void
SCRIPT_recursivefree(scriptnode_t* node)
{
  assert(node != NULL);
...
static void
SCRIPT_addsibling(scriptnode_t* node, scriptnode_t* sibling)
{
  assert(node != NULL);
  assert(sibling != NULL);
...
static void
SCRIPT_addchild(scriptnode_t* parent, scriptnode_t* child)
{
  assert(parent != NULL);
  assert(child != NULL);
...
static int
SCRIPT_getnexttoken(char* buffer, char* token, int start)
{
  int iterator = start;
...
static void
SCRIPT_parseline(char* curline, scriptnode_t* node)
{
  char token[128];
  int i;
...
static scriptnode_t*
SCRIPT_findinchildren(scriptnode_t* parent, char* s)
{
