...
int32_t
transword(void) // Returns its code #
{
  int32_t i, l;
...
void
loadefs(char* filenam, char* mptr, int readfromGRP)
{
  int32_t fs, fp;
  uint8_t kbdKey;
  ...
  *script = encodescriptptr(scriptptr);
  ...
  else
  {
    ...
    if (conVersion != 13 && (getGRPcrc32(0) == CRC_BASE_GRP_SHAREWARE_13 || getGRPcrc32(0) == CRC_BASE_GRP_FULL_13) && !getGRPcrc32(1))
    {
...
short
furthestangle(short i, short angs)
{
...
short
furthestcanseepoint(short i, spritetype* ts, int32_t* dax, int32_t* day)
{
    ...
    case 37:
    {
      ...
      if (g_sp->sectnum >= 0 && g_sp->sectnum < MAXSECTORS)
        for (j = (*insptr) - 1; j >= 0; j--)
        {
          ...
          l = EGS(g_sp->sectnum,
                  g_sp->x + (TRAND & 255) - 128,
                  g_sp->y + (TRAND & 255) - 128,
                  g_sp->z - (8 << 8) - (TRAND & 8191),
                  dnum + s,
                  g_sp->shade,
                  32 + (TRAND & 15),
                  32 + (TRAND & 15),
...
void
execute(short i, short p, int32_t x)
{
  uint8_t done;
