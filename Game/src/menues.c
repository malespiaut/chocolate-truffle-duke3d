...
void
savetemp(char* fn, uint8_t* daptr, int32_t dasiz)
{
  int fp;
...
probeXduke(int x, int y, int i, int n, int32_t spriteSize)
{
  ...
  if (KB_KeyPressed(sc_DownArrow) || KB_KeyPressed(sc_PgDn) || KB_KeyPressed(sc_kpad_2) || (mi > 1024) || WHEELDOWN)
  {
...
menus(void)
{
  ...
  switch (current_menu)
  {
    ...
    case 1010: // loading a saved game failed
      ...
      _handle_events();
      if (KB_KeyPressed(sc_Space) || KB_KeyPressed(sc_Escape) || KB_KeyPressed(sc_Enter) || RMB)
      {
        KB_ClearKeyDown(sc_Space);
        KB_ClearKeyDown(sc_Escape);
        KB_ClearKeyDown(sc_Enter);
    ...
    case 997:
        ...
        KB_KeyPressed(sc_kpad_6))
      {
        ...
        sound(KICK_HIT);
        current_menu++;
        if (current_menu > 990 + l)
          current_menu = 990;
      }

      if (!PLUTOPAK)
      { // This is Jonathon Fowler code. Text respects the true 1.3/1.3d --mk
        switch (current_menu)
        {
          case 990:
    ...
    case 50: // general menu as cmenu(0) but for multiplayer games
      ...
      if (numplayers > 1)
        ...
        menutext(c, 67 + 16 * 5, SHX(-7), PHX(-7), "QUIT TO TITLE");
...
if(boardfilename[0])
    ...
    case 200:
      ...
      else
      {
        ...
        if (ud.m_recstat == 1)
          ...
          menutext(c + 160 + 40, 43 + 16 + 16 + 16 + 16 + 16, SHX(-10), PHX(-10), "OFF");
      }
    ...
    case 700:
      ...
      if (VoiceToggle)
        menutext(c + 160 + 40, 43 + 16 + 16 + 16 + 16, 0, (FXDevice == SC_Unknown) || SoundToggle == 0, "ON");
      else
        menutext(c + 160 + 40, 43 + 16 + 16 + 16 + 16, 0, (FXDevice == SC_Unknown) || SoundToggle == 0, "OFF");

      if (AmbienceToggle)
        menutext(c + 160 + 40, 43 + 16 + 16 + 16 + 16 + 16, 0, (FXDevice == SC_Unknown) || SoundToggle == 0, "ON");
      else
        menutext(c + 160 + 40, 43 + 16 + 16 + 16 + 16 + 16, 0, (FXDevice == SC_Unknown) || SoundToggle == 0, "OFF");

      if (ReverseStereo)
        menutext(c + 160 + 40, 43 + 16 + 16 + 16 + 16 + 16 + 16, 0, (FXDevice == SC_Unknown) || SoundToggle == 0, "ON");
      else
        menutext(c + 160 + 40, 43 + 16 + 16 + 16 + 16 + 16 + 16, 0, (FXDevice == SC_Unknown) || SoundToggle == 0, "OFF");

      menutext(c, 43 + 16 + 16 + 16 + 16 + 16 + 16 + 16, SHX(-9), (FXDevice == SC_Unknown) || SoundToggle == 0, "OPPONENT SOUND");
      if (OpponentSoundToggle)
        menutext(c + 160 + 40, 43 + 16 + 16 + 16 + 16 + 16 + 16 + 16, 0, (FXDevice == SC_Unknown) || SoundToggle == 0, "ON");
      else
        menutext(c + 160 + 40, 43 + 16 + 16 + 16 + 16 + 16 + 16 + 16, 0, (FXDevice == SC_Unknown) || SoundToggle == 0, "OFF");
    ...
    case 701:
...

      {
        ...
        if (MouseAiming)
          menutext(c + 160 + 40, 43 + 16 * 2, SHX(-7), PHX(-7), "HELD");
        else
          menutext(c + 160 + 40, 43 + 16 * 2, SHX(-7), PHX(-7), "TOGGLE");

        menutext(c, 43 + 16 * 3, SHX(-7), MouseAiming, "MOUSE AIMING");
        if (myaimmode)
          menutext(c + 160 + 40, 43 + 16 * 3, SHX(-7), MouseAiming, "ON");
        else
          menutext(c + 160 + 40, 43 + 16 * 3, SHX(-7), MouseAiming, "OFF");

        menutext(c, 43 + 16 * 4, SHX(-7), PHX(-7), "MOUSE AIMING FLIP");
        if (ud.mouseflip)
          menutext(c + 160 + 40, 43 + 16 * 4, SHX(-7), PHX(-7), "ON");
        else
          menutext(c + 160 + 40, 43 + 16 * 4, SHX(-7), PHX(-7), "OFF");
    ...
    case 702:
      ...
      if (ud.shadows)
        menutext(c + 160 + 40, 43 + 16 * 0, 0, 0, "ON");
      else
        menutext(c + 160 + 40, 43 + 16 * 0, 0, 0, "OFF");

      menutext(c, 43 + 16 * 1, SHX(-4), PHX(-4), "SCREEN TILTING");
      switch (ud.screen_tilting)
      {
        case 0:
          menutext(c + 160 + 40, 43 + 16 * 1, 0, 0, "OFF");
          break;
        case 1:
          menutext(c + 160 + 40, 43 + 16 * 1, 0, 0, "ON");
          break;
        case 2:
          menutext(c + 160 + 40, 43 + 16 * 1, 0, 0, "FULL");
          break;
      }

      menutext(c, 43 + 16 * 2, SHX(-3), PHX(-3), "CINEMATICS");
      if (ud.showcinematics)
        menutext(c + 160 + 40, 43 + 16 * 2, 0, 0, "ON");
      else
        menutext(c + 160 + 40, 43 + 16 * 2, 0, 0, "OFF");

      menutext(c, 43 + 16 * 3, SHX(-3), PHX(-3), "WEAPON MODEL");
      if (ud.hideweapon)
        menutext(c + 160 + 40, 43 + 16 * 3, 0, 0, "OFF");
      else
        menutext(c + 160 + 40, 43 + 16 * 3, 0, 0, "ON");

      menutext(c, 43 + 16 * 4, SHX(-3), PHX(-3), "WEAPON SWITCH");
      if (ud.weaponautoswitch)
        menutext(c + 160 + 40, 43 + 16 * 4, 0, 0, "OFF");
      else
        menutext(c + 160 + 40, 43 + 16 * 4, 0, 0, "ON");

      switch (ud.auto_aim)
      {
        case 0:
          menutext(c, 43 + 16 * 5, 0, nHostForceDisableAutoaim, "AUTOAIM DISABLED BY HOST");
          break;
        case 1:
          menutext(c, 43 + 16 * 5, 0, 0, "AUTOAIM BULLET ONLY");
          break;
        case 2:
          menutext(c, 43 + 16 * 5, 0, 0, "AUTOAIM REGULAR FULL");
          break;
      }
    ...
    case 703:
      ...
      rotatesprite(0 << 16, 0 << 16, 65536l << 5, 0, BLANK, 8, 0, 1 + 2 + 8 + 16 + 64, scale(0, xdim, 320), scale(0, ydim, 200), scale(320 - 0, xdim, 320) - 1, scale(200 - 34, ydim, 200) - 1);
    ...
    case 704: // mouse button setup, frm menu 701
      ...
      // black translucent background underneath lists
      rotatesprite(0 << 16, 0 << 16, 65536l << 5, 0, BLANK, 8, 0, 1 + 2 + 8 + 16 + 64, scale(0, xdim, 320), scale(40, ydim, 200), scale(320 - 0, xdim, 320) - 1, scale(200 - 75, ydim, 200) - 1);
    ...
    case 705: // digital axes setup, from menu 701
      ...
      // black translucent background underneath lists
      rotatesprite(0 << 16, 0 << 16, 65536l << 5, 0, BLANK, 8, 0, 1 + 2 + 8 + 16 + 64, scale(0, xdim, 320), scale(40, ydim, 200), scale(320 - 0, xdim, 320) - 1, scale(200 - 100, ydim, 200) - 1);
    ...
    case 600: // multiplayer and bot menu
      ...
      switch (x)
      {
        ...
        case -4: // idle on case 2
          if (KB_KeyPressed(sc_kpad_4) || KB_KeyPressed(sc_LeftArrow))
          {
            ...
            if (ud.m_level_number < 0)
              ud.m_level_number = 10;
        ...
        case -3: // Idle on case 1
          if (!VOLUMEONE)
          {
            if (KB_KeyPressed(sc_kpad_4) || KB_KeyPressed(sc_LeftArrow))
            {
              ...
              if (PLUTOPAK)
              {
                if (ud.m_volume_number < 0)
                  ud.m_volume_number = 3;
              }
              else
              {
                if (ud.m_volume_number < 0)
      ...
      if (ud.m_coop == 1)
        ...
        menutext(c, 57 + 16 + 16 + 16 + 16 + 16 - 9, SHX(-6), 1, "FR. FIRE");
...
void
palto(uint8_t r, uint8_t g, uint8_t b, int32_t e, int present)
{
  int i;
...
drawoverheadmap(int32_t cposx, int32_t cposy, int32_t czoom, short cang)
{
  ...
  for (p = connecthead; p >= 0; p = connectpoint2[p])
  {
    ...
    if (p == screenpeek || ud.coop == 1)
    {
      ...
      rotatesprite((x1 << 4) + (xdim << 15), (y1 << 4) + (ydim << 15), j, daang, i, sprite[ps[p].i].shade, sprite[ps[p].i].pal, (sprite[ps[p].i].cstat & 2) >> 1, windowx1, windowy1, windowx2, windowy2);
    }
  }
}

void
endanimsounds(int32_t fr)
{
